...
Fraction BigDecimal::fraction()
{
	...
	BigDecimal i, j, n = DecimalToBigDecimal(a.getString().length());

	for (i = "0"; n > i; i++)
	{
		if (a.isDecimalPoint(BigDecimalToDecimal(i)))
		{
		...
		{
			numerator.getString().push_back(a.getString().charAt(BigDecimalToDecimal(i)));
		}
	}
